...
假设：精修子模型加入"以目标为中心"的相对位置编码（SDF 符号距离场 + 相对质心坐标
通道，CoordConv 风格）可校准位置相关的系统误差。实测：3D Dice 0.8894 (v1) →
0.8854 (v2)，噪声级差异，无增益。分析：粗 mask 本身已隐式携带位置/形状信息，位置
通道冗余；剩余误差为图像本身的边界模糊性，非位置相关系统偏差。结论：该场景下
子模型的信息瓶颈不在位置编码。

## 7. YOLO 类检测-分割范式评估（不采用）

YOLO-seg（v8/11-seg）掩码由低分辨率原型图线性组合（YOLACT 范式）生成，边界精度低于
编码器-解码器分割网络；本任务为单实例、边界敏感的逐病例 Dice 评估、离线处理，其速度
与多实例优势无用武之地。可借鉴形态是"检测器粗定位 → 分割器精分割"两段式，用于解决
部署时的自动定位问题（本研究评估采用 GT 质心定位的理想假设，部署可换粗定位/滑窗）。
