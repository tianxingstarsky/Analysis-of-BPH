...
| 模型 | 逐病例 3D Dice | 交互成本 |
|---|---|---|
| **U-Net + 显式背景监督（本项目）** | **0.9152** | 全自动 |
| U-Net 最优（无背景监督） | 0.8725 | 全自动 |
| 旧基线（修复前） | 0.7090 | 全自动 |
...
4. **显式背景监督是最大单项增益（+0.043）**：与其调增强，不如改监督信号的结构——
   让模型对背景承担显式的分割义务，而不是默认"推向 0"。

### 3.3 二次校准：传统形态学 vs 子模型精修

针对粗分割的系统误差，对比两种二次校准（粗分割 = 3.2 的最优模型，3D Dice 0.8725）：

| 校准方式 | 逐病例 3D Dice | 变化 |
|---|---|---|
| 传统形态学（最大连通域+闭运算+填洞） | 0.8726 | +0.0001（无增益） |
| **子模型精修**（2通道[图像+粗mask]轻量U-Net，25 epoch） | **0.8894** | **+0.017** ✅ |

精修效果可视化（每病例选取精修增益最大的切片）：

![精修效果](figures/fig_refiner_effect.png)

最典型的是 p06：粗分割完全失效（Dice 0.000，只输出一个点状预测），子模型将其恢复到
0.550——这类"漏检切片被找回"正是 3D Dice +0.017 的主要来源（体积口径下找回一个漏检
层对重叠体积的贡献远大于边界微调）。

**分析**：形态学零增益的原因是当前模型的预测已是干净单连通域，其误差为系统性边界/形状
偏差（光滑但形状略偏的 blob），形态学只能修碎片类错误；子模型精修（cascade/auto-context
范式）以"图像+粗mask"为输入，专门学习主模型的系统性偏差，4 例验证全部正向。
注意：子模型训练使用主模型在训练集上的预测作为粗 mask，存在轻微乐观偏差，严格结论
需以交叉验证粗 mask 重训。精修权重同在 Release v1.0。
...
解决方案：双 Sigmoid 通道显式监督——通道 0 以前景标注监督（学分割腺体），
通道 1 以反转标注监督（**学分割背景**），各自独立 BCE+Dice。相比 classes=2 的
softmax 竞争（两通道概率此消彼长，曾导致塌缩），独立 Sigmoid 让背景获得
明确的拒绝梯度，而不与前景抢概率预算。

| 模型 | 逐病例 3D Dice |
|---|---|
| U-Net + 显式背景监督 | **0.9152**（旧最优 0.8725，**+0.043**） |

分病例中层切片：p06 0.883（原完全失效病例）/ p18 0.950 / p24 **0.963** / p40 0.782。
p24 上甚至超过 MedSAM2 零样本（0.937）。
